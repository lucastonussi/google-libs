...
_THIRD_PARTY_LIBRARY_FORMAT_STRING = os.environ.get(
    'PYTHON_SANDBOX_THIRD_PARTY_LIBRARY_FORMAT_STRING',
...
class _ThirdPartyCModules(object):
  ...
  def get_importable_module_names(self, config):
    ...
    """
    return set(self._name_to_module[lib.name].import_name
...
THIRD_PARTY_C_MODULES = _ThirdPartyCModules([
    ...
    _ThirdPartyCModule('PIL',
                       r'(PIL(\..*)?|_imaging|_imagingft|_imagingmath)$'),
...
def add_open_hook(install_open_hook):
  ...
  Args:
    install_open_hook: a method of no parameters that will install an open hook.
  """
...
def enable_sandbox(config):
  ...
  from google.appengine.runtime import runtime  # pylint: disable=g-import-not-at-top
...
def _install_import_hooks(config, path_override_hook):
  ...
  Args:
    config: An apphosting/tools/devappserver2/runtime_config.proto for this
      instance.
    path_override_hook: A hook for importing special appengine versions of
      select libraries from the libraries section of the current module's
      app.yaml file.
  """
  if not config.vm:
    ...
    sys.meta_path = [
        ...
        CModuleImportHook(enabled_regexes), path_override_hook,
...
def _should_keep_module(name):
  ...
  return (
      name in ('__builtin__', '__main__', 'sys', 'codecs', 'encodings', 'site',
               'google', 'tempfile') or name.startswith('google.') or
      name.startswith('encodings.') or
...
def _enable_libraries(libraries):
  ...
  """
  library_dirs = []
  lib_parent_dir = os.path.dirname(
      os.path.dirname(os.path.dirname(os.path.dirname(google.__file__))))
  library_pattern = os.path.join(lib_parent_dir,
  ...
  for library in libraries:
    ...
    library_dir = os.path.abspath(
        library_pattern % {
            'name': library.name.encode('ascii'),
...
class PathOverrideImportHook(BaseImportHook):
  ...
  def __init__(self, modules):
    ...
    if 'grpc' in modules:
      ...
      grpc_dep_libraries = [
          'enum', 'six', 'concurrent.futures', 'google.protobuf'
      ]
        ...
        ModuleOverridePolicy(
            ...
            overrides={
                'access':
                    stubs.fake_access,
                'chmod':
                    stubs.RestrictedPathFunction(os.chmod, for_write=True),
                'listdir':
                    stubs.make_fake_listdir(os.listdir),
                # Alias lstat() to stat() to match the behavior in production.
                'lstat':
                    stubs.RestrictedPathFunction(os.stat),
                'makedirs':
                    stubs.RestrictedPathFunction(os.makedirs, for_write=True),
                'mkdir':
                    stubs.RestrictedPathFunction(os.mkdir, for_write=True),
                'open':
                    stubs.fake_open,
                'stat':
                    stubs.RestrictedPathFunction(os.stat),
                ...
                'getuid':
                    stubs.return_minus_one,
                'remove':
                    stubs.RestrictedPathFunction(os.remove, for_write=True),
                'rename':
                    stubs.RestrictedPathFunction(os.rename, for_write=True),
                'unlink':
                    stubs.RestrictedPathFunction(os.unlink, for_write=True),
                'urandom':
                    stubs.fake_urandom,
                'system':
                    stubs.return_minus_one,
